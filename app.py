# ============================================================
# CERÂMICAIÁ v1.0 — Dashboard Oficial
# ============================================================
...
st.markdown('<div class="sub-header">Previsão em tempo real de Resíduo, Retração, Peso e Cálculo de Perdas Financeiras</div>', unsafe_allow_html=True)
st.divider()
...
try:
    ...
    st.info("Certifique-se de que modelo_residuo.pkl, modelo_retracao.pkl e modelo_peso.pkl estão na mesma pasta.")
    st.stop()
...
# PAINEL LATERAL DE CONFIGURAÇÃO DO LOTE
...
# TELA PRINCIPAL - ENTRADA DE MISTURA
...
if modo == "Receita Única":
    ...
    with c1: preto_a = st.number_input("Conchas de PRETO (BSV)", min_value=0, max_value=10, value=4, step=1)
    with c2: amarelo_a = st.number_input("Conchas de AMARELO", min_value=0, max_value=10, value=0, step=1)
    with c3: branco_a = st.number_input("Conchas de BRANCO", min_value=0, max_value=10, value=1, step=1)
    ...
    c1, c2, c3 = st.columns(3)
    with c1: preto_a = st.number_input("Preto (A)", min_value=0, max_value=10, value=4, step=1)
    with c2: amarelo_a = st.number_input("Amarelo (A)", min_value=0, max_value=10, value=0, step=1)
    with c3: branco_a = st.number_input("Branco (A)", min_value=0, max_value=10, value=1, step=1)
    
    st.subheader("Receita B")
    c4, c5, c6 = st.columns(3)
    with c4: preto_b = st.number_input("Preto (B)", min_value=0, max_value=10, value=4, step=1)
    with c5: amarelo_b = st.number_input("Amarelo (B)", min_value=0, max_value=10, value=0, step=1)
    with c6: branco_b = st.number_input("Branco (B)", min_value=0, max_value=10, value=2, step=1)
...
with col_u:
    umidade = st.number_input("Umidade do Barro (%)", min_value=5.0, max_value=30.0, value=16.0, step=0.5)
with col_e:
    esp_parede = st.number_input("Espessura da Parede (cm)", min_value=0.20, max_value=1.50, value=0.65, step=0.01)
...
if st.button("🔮 GERAR DIAGNÓSTICO DO LOTE", type="primary", use_container_width=True):
    ...
    if excesso_g > 0:
        ...
        with c_p1:
            prod_dia = st.number_input("Produção Planejada do Dia (blocos):", min_value=1000, max_value=200000, value=50000, step=5000)
        with c_p2:
            custo_barro = st.number_input("Custo da Tonelada do Barro (R$/ton):", min_value=10.0, max_value=200.0, value=50.0, step=5.0)
        ...
        """)
        st.info("💡 **Ação Recomendada:** Reduza a espessura da parede ou ajuste a proporção para retornar ao peso meta.")
