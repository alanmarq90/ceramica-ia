# ============================================================
# CERAMICAIA v13.0 — Hibrido (Fisica + IA) | Isolamento de Peso
# ============================================================
...
# ============================================================
# IA AUTO-APRENDIZ: MODELO HIBRIDO (FISICA + ML)
# ============================================================
...
def treinar_modelos_ia(df_lotes, df_produtos, df_barros, df_puro):
    ...
    if n < 5:
        return None, None, None, {"n": n, "r2_peso": None, "r2_residuo": None, "r2_retracao": None}
    ...
    # 2. Treino do Modelo de RESIDUO (Prevê apenas o DELTA do Processo)
...
with st.sidebar.expander("Detalhes do treinamento da IA"):
    st.write(f"R2 Peso (Geometria): {metricas_ia['r2_peso']}")
    st.write(f"R2 Residuo (Hibrido): {metricas_ia['r2_residuo']}")
    st.write(f"R2 Retracao (Padrao): {metricas_ia['r2_retracao']}")
    st.caption("A IA de Residuo usa Fisíca + ML para evitar limites de extrapolação.")
...
with tab_diag:
    ...
    # Lógica de interface intocada da v12.1 original
    ...
    if st.session_state.diagnostico_gerado:
        ...
        # O modelo de resíduo prevê apenas o DELTA. O resíduo final é a Base + Delta
        pred_delta_res = m_res.predict(X_input_res_ml)[0]
        pred_res = res_puro_ponderado_entrada + pred_delta_res
