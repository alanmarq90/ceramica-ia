# ============================================================
# CERAMICAIA v14.1 — Hibrido (Misturas + Barros Puros) | Fix indent
# ============================================================
import io
import os
import urllib.parse
...
if "df_master" not in st.session_state:
    if MODO_SHEETS and worksheet_lotes:
        ...
        if len(df_l) == 0:
            ...
            st.toast("Google Sheets estava vazio: lotes historicos inseridos com sucesso!")
...
def obter_residuo_puro_barro(cod_barro, df_barros, df_puro):
    ...
    try:
        if df_puro is not None and len(df_puro) > 0 and "codigo_barro" in df_puro.columns:
    ...
    try:
        if df_barros is not None and len(df_barros) > 0 and "codigo" in df_barros.columns:
            df_filtro_b = df_barros[df_barros["codigo"] == cod_barro]
            if len(df_filtro_b) > 0:
                row_b = df_filtro_b.iloc[0]
                val_cad = pd.to_numeric(row_b.get("residuo_puro", 0.0), errors="coerce")
                if not pd.isna(val_cad) and float(val_cad) > 0:
                    return float(val_cad)
                tipo = row_b.get("tipo_base", "")
                if tipo == "Preto":
                    return 10.0
                if tipo == "Amarelo":
                    return 25.0
...
# ============================================================
# IA AUTO-APRENDIZ: MODELO HIBRIDO
# ============================================================
...
def montar_dataset_treino(df_lotes, df_produtos, df_barros, df_puro):
    ...
    try:
        ...
        df["largura_cm"] = pd.to_numeric(df["largura_cm"], errors="coerce")
    except Exception:
...
def treinar_modelo_barros_puros(df_montado):
    if df_montado is None or len(df_montado) < 5:
        return None, {"r2_puro": None}
    cols_needed = FEATURES_ML_RESIDUO + ["residuo_puro_ponderado", "residuo"]
    df_puro_treino = df_montado.dropna(subset=cols_needed)
    if len(df_puro_treino) < 5:
        return None, {"r2_puro": None}
    X_puro = df_puro_treino[FEATURES_ML_RESIDUO].astype(float)
    y_puro = (df_puro_treino["residuo"].astype(float) - df_puro_treino["residuo_puro_ponderado"].astype(float))
    m_puro = GradientBoostingRegressor(random_state=42)
    m_puro.fit(X_puro, y_puro)
    pred_delta_puro = m_puro.predict(X_puro)
    pred_final_puro = df_puro_treino["residuo_puro_ponderado"].astype(float) + pred_delta_puro
    try:
        r2_puro = r2_score(df_puro_treino["residuo"].astype(float), pred_final_puro)
    except Exception:
        r2_puro = 0.0
    return m_puro, {"r2_puro": round(float(r2_puro), 3)}
...
def treinar_modelos_ia(df_lotes, df_produtos, df_barros, df_puro):
    ...
    metricas = {
        "n": n,
        "r2_peso": round(float(r2_pes), 3),
        "r2_residuo": round(float(r2_res), 3),
        "r2_retracao": round(float(r2_ret), 3),
    }
    # 4. Modelo puro - so ativa se ja tem analise pura real (Opcao A)
    if df_puro is None or len(df_puro) == 0:
        m_puro = None
        metricas_puro = {"r2_puro": None}
    else:
        m_puro, metricas_puro = treinar_modelo_barros_puros(df)
    metricas.update(metricas_puro)
    return m_pes, m_res, m_ret, m_puro, metricas


if ("modelos_ia" not in st.session_state or st.session_state.get("versao_treinada") != st.session_state.versao_dados):
...
if m_pes is None:
    st.error(
        f"Dados insuficientes para treinar a IA (apenas {metricas_ia['n']} lotes validos encontrados). Registre pelo menos 5 lotes completos."
    )
    st.stop()
...
with st.sidebar.expander("Detalhes do treinamento da IA"):
    st.write("Modelo Empirico (Misturas):")
    ...
    if metricas_ia.get('r2_puro') is not None:
        st.write("Modelo Fisico (Barros Puros):")
        st.write(f"  R2 Residuo Puro: {metricas_ia['r2_puro']}")
    else:
        st.write("Modelo Fisico (Barros Puros): Aguardando dados... faca analises na Aba 3")
    st.caption("A IA combina Empirico (historico) + Fisico (barros puros) para maxima precisao.")
...
with tab_diag:
    ...
    with col_c:
        comprimento_cm = st.number_input(
            ...
            help=f"Tamanho ideal verde para este produto e {comp_seco_ideal:.1f} cm. Se a guilhotina cortar maior, o peso aumenta.",
        )
    st.divider()
    ...
    if st.session_state.diagnostico_gerado:
        ...
        if tem_modelo_puro:
            ...
            confianca_residuo = "ALTA (Ambos modelos)"
        else:
            pred_res = pred_res_empirico
            confianca_residuo = "MEDIA (So empirico)"
        ...
        if diff_comp > 0.15:
            st.warning(
                f"ALERTA DE CORTE NA EXTRUSORA: Bloco verde cortado com {comprimento_cm:.1f} cm (+{diff_comp*10:.0f} mm acima do ideal de {comp_seco_ideal:.1f} cm). Esse excesso aumenta o peso. Ajuste a guilhotina do carretel."
            )
        elif diff_comp < -0.15:
            st.warning(
                f"ATENCAO AO CORTE NA EXTRUSORA: Bloco verde cortado com {comprimento_cm:.1f} cm (-{abs(diff_comp)*10:.0f} mm abaixo do ideal de {comp_seco_ideal:.1f} cm). Risco de ficar curto apos a queima."
            )
        if tem_modelo_puro:
            st.subheader("Analise Detalhada de Modelos")
            col_m1, col_m2, col_m3 = st.columns(3)
            with col_m1:
                st.metric("Modelo Empirico (Misturas)", f"{pred_res_empirico:.1f}%")
                st.caption(f"Baseado em {metricas_ia['n']} lotes historicos")
            with col_m2:
                st.metric("Modelo Fisico (Barros Puros)", f"{pred_res_puro:.1f}%")
                st.caption(f"R2 = {metricas_ia['r2_puro']}")
            with col_m3:
                st.metric("PREVISAO FINAL (Media)", f"{pred_res:.1f}%")
                st.caption("Hibrida: Empirico + Fisico")
            st.divider()
        else:
            st.info("Modelo Fisico ainda indisponivel: Faca analises de barros puros na Aba 3 para ativar o modelo hibrido!")

        res1, res2, res3 = st.columns(3)
        with res1:
            if tem_modelo_puro:
                st.metric("Residuo Previsto no Bloco Queimado (FINAL)", f"{pred_res:.1f}%")
            else:
            ...
            st.caption(msg_res)
            if tem_modelo_puro:
                st.caption(f"Confianca: {confianca_residuo}")
        with res2:
        ...
        if excesso_g > 0:
            ...
            texto_fin_wa = f"\nPrejuizo estimado: R$ {prejuizo_dia:,.0f}/dia ({ton_perdidas_dia:.1f} ton desperdicadas)"
        st.divider()
        if tem_modelo_puro:
            modelos_info = (
                f"Modelos:\n"
                f"- Empirico (Misturas): {pred_res_empirico:.1f}%\n"
                f"- Fisico (Barros Puros): {pred_res_puro:.1f}%\n"
                f"- FINAL (Media): {pred_res:.1f}%\n"
            )
        else:
            modelos_info = f"Modelo: Empirico (Misturas)\n- Residuo: {pred_res:.1f}%\n"
        msg_wa_diag = (
...
with tab_reg:
    ...
    st.caption("Alimente o sistema com os dados medidos nas amostras de laboratorio. A IA recalibra automaticamente apos o salvamento.")
    ...
    with st.form("form_registro_lote", clear_on_submit=True):
        ...
        with f_con2:
            ...
            p_b = st.number_input("Preto B", 0, 10, 4)
            a_b = st.number_input("Amarelo B", 0, 10, 0)
            b_b = st.number_input("Branco B", 0, 10, 1)
        ...
        with f_col8:
            comp_real_medido = st.number_input("Comprimento Verde de Corte Medido (cm):", 10.0, 50.0, float(comp_seco_sugerido), 0.1, help="Tamanho medido com paquimetro/trena no bloco verde.")
        with f_col9:
            peso_real = st.number_input("Peso Real Medido (kg)", 0.0, 15.0, 3.100, 0.001)
        with f_col10:
            obs_texto = st.text_area("Observacoes:", "Teste de rotina")
        btn_salvar = st.form_submit_button("Salvar Registro e Recalibrar IA", type="primary", use_container_width=True)
    ...
    if btn_salvar:
        ...
        st.divider()
        st.subheader("Analise Real vs Previsao")
        col_comp1, col_comp2, col_comp3 = st.columns(3)
        with col_comp1:
            st.metric("Residuo Real Medido", f"{residuo_real:.1f}%")
            st.caption("Resultado do laboratorio")
        with col_comp2:
            st.metric("Modelo Empirico", f"{pred_res_emp_l:.1f}%")
            st.caption(f"Erro: {abs(residuo_real - pred_res_emp_l):.1f}%")
        with col_comp3:
            if tem_puro_l:
                st.metric("Modelo Fisico", f"{pred_res_puro_l:.1f}%")
                st.caption(f"Erro: {abs(residuo_real - pred_res_puro_l):.1f}%")
            else:
                st.metric("Previsao Final", f"{pred_res_l:.1f}%")
                st.caption(f"Erro: {abs(residuo_real - pred_res_l):.1f}%")
        if tem_puro_l:
            erro_final = abs(residuo_real - pred_res_l)
            if erro_final < 1.0:
                st.success(f"PREVISAO FINAL: {pred_res_l:.1f}% | Erro: {erro_final:.2f}% ACERTOU!")
            elif erro_final < 2.0:
                st.info(f"PREVISAO FINAL: {pred_res_l:.1f}% | Erro: {erro_final:.2f}% Bom!")
            else:
                st.warning(f"PREVISAO FINAL: {pred_res_l:.1f}% | Erro: {erro_final:.2f}% Analisar")
            msg_previsoes = (
                f"Previsoes:\n"
                f"- Empirico: {pred_res_emp_l:.1f}%\n"
                f"- Fisico: {pred_res_puro_l:.1f}%\n"
                f"- Final: {pred_res_l:.1f}%\n"
                f"- Real: {residuo_real}% | Erro: {abs(residuo_real - pred_res_l):.1f}%\n"
            )
        else:
            msg_previsoes = (
                f"Previsao: {pred_res_l:.1f}%\n"
                f"Real: {residuo_real}% | Erro: {abs(residuo_real - pred_res_l):.1f}%\n"
            )
        msg_wa_reg = (
    ...
    with col_prod2:
        ...
        if len(lista_produtos_cod) > 0:
            ...
            with st.form("form_editar_produto"):
                ...
                with ce_dim3:
                    edit_prod_comp_sec = st.number_input("Comprimento Verde Ideal de Corte na Extrusora (cm):", 5.0, 55.0, float(dados_prod_atual["comp_seco_ideal"]), 0.5)
